...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.getElementById('home-button')?.addEventListener('click', (e) => {
    home(e);
  }, {capture: true}); // This makes it execute first
...
function setupTrayPage() {
  ...
  const checkoutButton1 = document.getElementById("checkout-Button1");
  ...
  if (checkoutButton1) checkoutButton1.addEventListener("click", handleCheckout);
...
function home(e) {
  ...
  // Completely replace the history stack with index.html
  window.location.replace("index.html?" + new Date().getTime());
}

// In mytray.js, update the renderTrayItems function
function renderTrayItems() {
  ...
  if (window.innerWidth <= 768) {
    trayItems.innerHTML = `
      <nav class="menu-list">
        ${window.cart.items.map((item, index) => `
          ...
          ${item.note ? `
            <div class="mobile-note" style="padding: 10px; font-size: 14px; color: #666; background: #f5f5f5; border-radius: 5px; margin: 5px 0;">
          ...
          <div class="menu-item2">
              ...
              <div class="item-info2">
                  ...
                  <div class="note-to-chef" style="margin-top: 10px;">
                      <input type="text" placeholder="Note to Chef" value="${item.note || ""}" 
...
function handleTrayItemClick(itemId) {
  // Set a flag indicating we're coming from MyTray
  localStorage.setItem("fromMyTray", "true");
