...
// In reservation.js, modify the processReservation function
function processReservation(paymentMethod) {
  ...
  // Get cart items with notes
  const cart = JSON.parse(localStorage.getItem("wadau-cart") || []);
  const cartWithNotes = cart.map(item => {
    return {
      ...item,
      note: item.note || "" // Include the note if it exists
    };
  });

  const cartTotal = cartWithNotes.reduce((total, item) => total + (item.price * item.quantity), 0);
  ...
  const reservation = {
    ...
    status: paymentMethod === "Pay Now" ? "processing" : "awaiting",
    amount: cartTotal,
    items: cartWithNotes, // Include the cart items with notes
