...
function goBack() {
  // Only fade out if we're not already in a transition
...
document.addEventListener('DOMContentLoaded', () => {
  // Immediately set opacity to 1 to prevent white screen
  ...
  window.addEventListener('popstate', (event) => {
    // Only fade out if we have state to restore
    if (event.state) {
      document.body.style.opacity = '0';
      setTimeout(() => {
...
// Optimized findItemById with early returns
...
window.addEventListener("cartUpdated", () => {
  ...
  // Update all desktop items
  document.querySelectorAll('.menu-item2').forEach(itemElement => {
    const itemId = itemElement.querySelector('button.add-to-tray2')?.id.replace('btn-', '')
    if (itemId) {
      const button = document.getElementById(`btn-${itemId}`)
      const quantityElement = document.getElementById(`quantity-${itemId}`)
      const priceElement = document.getElementById(`price-${itemId}`)
      ...
      if (cartItem && originalItem) {
        ...
        // Update quantity display if different
        if (quantityElement && Number(quantityElement.textContent) !== cartItem.quantity) {
          quantityElement.textContent = cartItem.quantity
          priceElement.textContent = originalItem.price * cartItem.quantity
        }
      } else if (button) {
        // Item not in cart
        button.textContent = "Add to tray"
