...
// Navigation
function goBack() {
  // Check if this was triggered by a home button click
  if (event && event.target.closest('#home-button, .back[href="index.html"]')) {
    return; // Let the home button handler take care of it
  }
  const fromMyTray = localStorage.getItem("fromMyTray") === "true";
  
  if (fromMyTray && window.innerWidth <= 768) {
    // Coming from MyTray in mobile view - go back to MyTray
    localStorage.removeItem("fromMyTray");
...
function showMobileItem(item) {
  ...
  if (window.innerWidth <= 768) {
    ...
    mobileAddBtn.style.border = isInCart ? "2px solid black" : "none";
      ...
      //the note input
    let noteContainer = document.querySelector('.mobile-note-container');
    if (!noteContainer) {
      noteContainer = document.createElement('div');
      noteContainer.className = 'mobile-note-container';
      mobileDisplay.appendChild(noteContainer);
    }
    ...
    const noteInput = noteContainer.querySelector('.mobile-note-input');
    noteInput.addEventListener('change', () => {
      const note = noteInput.value;
      const itemId = noteInput.dataset.itemId;
      saveMobileNote(itemId, note);
    });
    function saveMobileNote(itemId, note) {
    ...
    // Reset info button state when showing new item
    document.querySelector(".info-text").style.display = "block";
    document.querySelector(".close-text").style.display = "none";
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.getElementById("mobile-add-to-tray")?.addEventListener("click", function() {
    ...
    const noteInput = document.querySelector('.mobile-note-input');
    const note = noteInput ? noteInput.value : "";

    const isInCart = window.cart.items.find((cartItem) => cartItem.id === currentMobileItem.id);

    if (isInCart) {
      // Update existing item with new note
